...
const TransformCard: React.FC<TransformCardProps> = ({index, length = 0}) => {
    return (
        <div className={`cursor-pointer transform transition-transform duration-200 scal relative shadow-custom-md  h-[98px] p-2 ${index == 0 ? "rounded-tl-[30px] rounded-b rounded-tr" : index == 1 ? "rounded-tr-[30px] rounded-b rounded-tl" : index == length -2 ? "rounded-bl-[30px] rounded-br rounded-t  " : index == length -1 ? "rounded-br-[30px] rounded-bl rounded-t" : " rounded" } `} >
            ...
            <p className='absolute text-white 431max:top-[1px] top-[15%] 431max:left-[40%] left-[50%] translate-x-[-50%] text-lg font-medium text-center' >
